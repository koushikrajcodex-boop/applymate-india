...
const els = {
  ...
  linkUrl: $("linkAnalyzerUrl"), bulkUrls: $("linkBulkUrls"), pdfFile: $("linkPdfFile"), fallbackText: $("linkFallbackText"),
  source: $("botPanelSource"), url: $("botPanelUrl"), input: $("botPanelInput"), message: $("botPanelMessage"), results: $("botPanelResults"),
  refresh: $("botPanelRefreshBtn"), fetchLink: $("linkAnalyzeBtn"), analyzeBulk: $("linkBulkAnalyzeBtn"), analyzePdf: $("linkPdfAnalyzeBtn"), analyzePasted: $("linkPastedAnalyzeBtn"), analyze: $("botPanelAnalyzeBtn"), clear: $("botPanelClearBtn"), importActive: $("botPanelImportActiveBtn"), importDraft: $("botPanelImportDraftBtn"), importSafeDrafts: $("botPanelImportSafeDraftsBtn")
};
...
async function loadCoverage() {
  ...
  try {
    ...
    setMessage("Coverage loaded. Use single link, bulk links, PDF upload, or pasted official text.");
...
async function analyzeSingleLink() {
  ...
  try {
    ...
    setMessage("Single source analyzed. Review confidence and missing fields before import.");
  } catch (error) {
    console.warn(error);
...
async function analyzeBulkLinks() {
  ...
  setMessage(`Bulk finished. ${out.length} records generated. High-confidence records can be imported; blocked ones stay as review drafts.`);
}

async function analyzeUploadedPdf() {
  const file = els.pdfFile?.files?.[0];
  if (!file) return setMessage("Choose an official PDF file first.", true);
  if (!/\.pdf$/i.test(file.name) && file.type !== "application/pdf") return setMessage("Upload PDF only.", true);
  ...
  try {
    ...
    setMessage("Extracting PDF text...");
    ...
    if (quality === "scanned_or_poor_text_pdf" || isWeakText(text, url)) {
      ...
      const ocr = await ocrPdf(buffer, OCR_PAGE_LIMIT);
    ...
    setValue(els.fallbackText, text.slice(0, 20000));
    setGeneratedCandidates([buildCandidate({ url, text, sourceName, origin: quality })]);
    setMessage(`PDF analyzed successfully (${quality.replaceAll("_", " ")}). Review before import.`);
  } catch (error) {
    console.error(error);
    setMessage("Could not read enough text from this PDF. Try OCR externally, then paste that text in the fallback box.", true);
  }
}
...
function analyzePastedText() {
  ...
  setMessage("Pasted official text analyzed with confidence, missing fields, documents, and summary.");
}
...
async function ocrPdf(buffer, maxPages) {
  ...
  for (let pageNo = 1; pageNo <= total; pageNo += 1) {
    setMessage(`OCR reading page ${pageNo}/${total}...`);
    const page = await pdf.getPage(pageNo);
...
function buildCandidate({ url, text, sourceName, origin }) {
  ...
  const name = guessTitle(cleanText, url);
  ...
  return normalize({
    ...
    documentType: detectDocumentType(cleanText, url, origin), sourceType: origin, rawSectionNames: Object.keys(sections)
  });
}
...
function classify(item) {
  ...
  else if (!looseErrors.some((x) => !x.toLowerCase().includes("deadline")) && confidence.overall >= 68 && isValidUrl(enriched.link)) decision = "autoDraft";
...
function confidenceScore(item, missingFields, safety, duplicate) {
  const score = {
    ...
    source: isValidUrl(item.link) ? (safety.level === "official" ? 100 : safety.level === "trusted" ? 82 : 55) : 0,
...
function render() {
  ...
  if (!candidates.length) return els.results.innerHTML = "<p class='mini-note'>No results yet.</p>";
...
async function importRecords(mode) {
  ...
  else if (mode === "draft") records = candidates.filter((x) => ["autoDraft", "review"].includes(x.decision));
  else records = candidates.filter((x) => ["autoActive", "autoDraft", "review"].includes(x.decision));
...
function guessTitle(text, url) { if (/Additional Scholarship.+OBC.+Andaman.+Nicobar/i.test(text)) return "Additional Scholarship to OBC Students of Andaman and Nicobar Islands"; const match = clean(text).match(/(?:scheme\s+for\s+grant\s+of|scholarship\s+name|name\s+of\s+the\s+scheme)\s*:?\s*(.{12,170}?)(?:,|\.|\s+for\s+pursuing|\n)/i); if (match) return titleCase(match[1]); const strong = clean(text).split(/\n|\.|\||•/).map(clean).find((x) => x.length >= 8 && x.length <= 150 && /scholarship|scheme|fellowship|grant|stipend/i.test(x) && !/login|copyright|official website|page \d+/i.test(x)); return strong || placeholderTitle(url); }
